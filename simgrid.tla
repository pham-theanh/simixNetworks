...
Actions == {\* All local actions are abstracted as a unique type
	   ...
	   \* Syncronization actions
	    "AsyncMutexLock", "MutexUnlock", "MutexTest", "MutexWait"
	   }
NoActor == "NoActor"
NoAddress == "NoAddress"
          
VARIABLES Communications, Memory, Mutex, Locks, Mailboxes, AsyncCount

(* Initially there are no Communications, no Locks on the mutexes, Memory has random values *)
...
TypeOK == /\ \forall c \in Communications : c \in Comm /\ c.status = "done" 
...
Constraint == /\ Cardinality(Communications) \leq 3
...
Local(aId) ==
    ...
    /\ UNCHANGED <<Communications, Mutex, Locks, Mailboxes, AsyncCount >>
...
(* AsyncSend(aId,mbId,data_addr):  
    ...
    If a pending "receive" request already exists,
      they are combined to create a "done" paired communication in Communications and data is copied from the source to the destination 
...
(* Wait(aId,comms):  Actor <aId> waits for at least one communication from a given set <comms> to complete 
 ...
 Parameters:
    ...
    - comms: list of communications on which the Actor is waiting
*)

WaitAny(aId, comms) ==
  /\ aId \in Actors
  /\ \E comm \in comms: comm \in Communications
  /\ UNCHANGED <<Mutex, Locks, Mailboxes, Memory, Communications, AsyncCount >>
...
(* Test(aId, comms, testResult_Addr): Actor <aId> waits for at least one communication from a given list <comms> to complete,
    ...
    - comms: the list of communications on which we test
...
\*  /\ testResult_Addr \in Addresses
  (* If one of the communications is "done" return TRUE; else return FALSE *)
  /\ \/ /\ \E comm \in comms: comm.status = "done"
        /\ Memory' = [Memory EXCEPT ![aId][testResult_Addr] = 1] (*True*)
     \/ /\ ~ \exists comm \in comms: comm.status = "done"
...
*) 
 ...
 MutexAsyncLock(aId, mId, req_addr) ==
   ...
   /\ AsyncCount' = AsyncCount+1	    
   /\ UNCHANGED <<Communications, Mailboxes>>
...
MutexUnlock(aId, mId) ==
   ...
   /\ UNCHANGED <<Memory, Communications, Mailboxes, AsyncCount>>
...
/\ \E req \in Locks[aId]: req = Memory[aId][req_addr] /\ isHead(aId, Mutex[req])
/\ UNCHANGED << Memory, Mutex, Locks, Communications, Mailboxes, AsyncCount>>
...
(* MutexTest(aId,req_addr, result_addr): actor <aId> tests if he is the owner of the mutex whose id is store in <req_addr>
            ...
            MutexTest is non-blocking
            *)
MutexTest(aId, req_addr, testResult_Addr) ==
  /\ aId \in Actors
  /\ testResult_Addr \in Addresses
  /\  \E req \in  Locks[aId]: req = Memory[aId][req_addr]/\ 
       (* If the actor is the owner then return true *)
        \/ /\ isHead(aId, Mutex[req])
           /\ Memory' = [Memory EXCEPT ![aId][testResult_Addr] = 1 (*TRUE*)]
       (*Else if it is not the onwer then return false *)
        \/ /\ ~isHead(aId, Mutex[req])
           /\ Memory' = [Memory EXCEPT ![aId][testResult_Addr] = 0 (*FALSE*)] 
  /\ UNCHANGED <<Mutex, Locks, Communications, Mailboxes, AsyncCount>>
...
Next ==   \/ \E actor \in Actors, mb \in MailboxesIds, addr \in Addresses:
          ...
          \/ \E actor \in Actors, comms \in SUBSET Communications:
	     WaitAny(actor, comms)
          \/ \E actor \in Actors, comms \in SUBSET Communications, addr \in Addresses:
	     TestAny(actor, comms, addr)
\*          \/ \E actor \in Actors:
\*	     Local(actor) 
          \/ \E actor \in Actors, mutex \in MutexesIds, addr \in Addresses:
	     MutexAsyncLock(actor, mutex, addr)
          \/ \E actor \in Actors, addr \in Addresses:
	     MutexWait(actor, addr)
          \/ \E actor \in Actors, addr1, addr2 \in Addresses:
	     MutexTest(actor, addr1, addr2) 
...
THEOREM \forall a1, a2 \in Actors, comms1, comms2 \in SUBSET Communications, test_r1, test_r2 \in Addresses:
        a1 /= a2 => 
...
THEOREM \forall a1, a2 \in Actors, mt \in MutexesIds, req,  data, comm, test_r1,  test_r2 \in Addresses,
         mbId\in MailboxesIds, comm_addrs \in SUBSET Addresses:
           ...
           /\ I(MutexWait(a1, req), AsyncSend(a2, mbId, data))
           /\ I(MutexWait(a1, req), AsyncReceive(a2, mbId, data))
           /\ I(MutexWait(a1, req), WaitAny(a2, comm_addrs))
           /\ I(MutexWait(a1, req), TestAny(a2, comm_addrs, test_r1))
          
           /\ I(MutexTest(a1, req, test_r1), AsyncSend(a2, mbId, data))
           /\ I(MutexTest(a1, req, test_r1), AsyncReceive(a2, mbId, data))
...
-------------------------------------------------
Spec == /\ Init
        /\ [][Next]_<< Communications, Memory, Mutex, Locks, Mailboxes, AsyncCount >>
