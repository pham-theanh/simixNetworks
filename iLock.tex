...
\@x{}\moduleLeftDash\@xx{ {\MODULE} iLock}\moduleRightDash\@xx{}%
\@x{ {\EXTENDS} Integers ,\, Naturals ,\, Sequences ,\, FiniteSets}%
\@pvspace{8.0pt}%
\@x{ {\CONSTANT} Proc ,\, Mutex ,\, ValTrue ,\, ValFalse ,\, Addr ,\,}%
\@x{\@s{54.27} UnlockIns ,\, MtestIns ,\, MwaitIns ,\, IlockIns}%
\@x{ {\VARIABLES} pc ,\, waitedQueue ,\, memory ,\, request}%
\@pvspace{8.0pt}%
 \@x{ Partition ( S ) \.{\defeq} \forall\, x ,\, y \.{\in} S \.{:} x \.{\cap}
 y \.{\neq} \{ \} \.{\implies} x \.{=} y}%
 \@x{ Instr \.{\defeq} {\UNION} \{ IlockIns ,\, UnlockIns ,\, MtestIns ,\,
 MwaitIns \}}%
\@pvspace{8.0pt}%
 \@x{ isHead ( m ,\, q ) \.{\defeq} {\IF} q \.{=} {\langle} {\rangle}
 \.{\THEN} {\FALSE}}%
\@x{\@s{81.24} \.{\ELSE} {\IF} m \.{=} Head ( q ) \.{\THEN}\@s{4.1} {\TRUE}}%
\@x{\@s{112.55} \.{\ELSE} {\FALSE}}%
\@pvspace{16.0pt}%
 \@x{ isContain ( m ,\, q ) \.{\defeq} {\IF} \E\, i \.{\in} ( 1 \.{\dotdot}
 Len ( q ) ) \.{:} m \.{=} q [ i ] \.{\THEN} {\TRUE}}%
\@x{\@s{98.34} \.{\ELSE} {\FALSE}}%
\@pvspace{16.0pt}%
\@x{ lockRequest \.{\defeq} [ id \.{:} Nat ,\,}%
\@x{\@s{71.82} process \.{:} Proc ,\,}%
\@x{\@s{71.82} mutex \.{:} Mutex}%
\@x{\@s{45.1} ]}%
\@pvspace{24.0pt}%
\@x{ Ilock ( pid ,\, mid ,\, req\_r ) \.{\defeq}}%
\@x{\@s{12.29} \.{\land} pid \.{\in} Proc}%
\@x{\@s{12.29} \.{\land} pc [ pid ] \.{\in} IlockIns}%
\@x{\@s{12.29} \.{\land} mid \.{\in} Mutex}%
\@x{\@s{12.29} \.{\land} req\_r \.{\in} Addr}%
\begin{lcom}{7.5}%
\begin{cpar}{0}{T}{F}{7.5}{0}{}%
asssume I can not do ilock if I have a request of
\end{cpar}%
\begin{cpar}{1}{F}{F}{0}{0}{}%
ilock of the same \ensuremath{mutex} need to be processed
\end{cpar}%
\end{lcom}%
\@x{\@s{12.29} \.{\land} {\lnot} isContain ( waitedQueue [ mid ] ,\, pid )}%
\@x{\@s{12.29} \.{\land} \.{\LET}\@s{4.1} req \.{\defeq}}%
\@x{\@s{73.48} [ id \.{\mapsto} Cardinality ( request ) \.{+} 1 ,\,}%
\@x{\@s{76.26} process \.{\mapsto} pid ,\,}%
\@x{\@s{76.26} mutex \.{\mapsto} mid ]}%
 \@x{\@s{23.41} \.{\IN} \.{\land} request \.{'}\@s{4.1} \.{=} request \.{\cup}
 \{ req \}}%
 \@x{\@s{43.81} \.{\land} memory \.{'} \.{=} [ memory {\EXCEPT} {\bang} [ pid
 ] [ req\_r ] \.{=} req . id ]\@s{4.1}}%
\@y{%
 store request\mbox{'}s \ensuremath{id} in the memory
}%
\@xx{}%
 \@x{\@s{12.29} \.{\land} \E\, ins \.{\in} Instr \.{:} pc \.{'} \.{=} [ pc
 {\EXCEPT} {\bang} [ pid ] \.{=} ins ]}%
 \@x{\@s{12.29} \.{\land} waitedQueue \.{'} \.{=} [ waitedQueue {\EXCEPT}
 {\bang} [ mid ] \.{=} Append ( waitedQueue [ mid ] ,\, pid ) ]}%
\@pvspace{8.0pt}%
\@x{}%
\@y{%
 unlock does not need to follow a \ensuremath{Mtest} or \ensuremath{Mwait} in
 this version
}%
\@xx{}%
\@pvspace{8.0pt}%
\@x{ Unlock ( pid ,\, mid ) \.{\defeq}}%
\@x{\@s{12.29} \.{\land}\@s{7.07} pid\@s{3.06} \.{\in} Proc}%
\@x{\@s{12.29} \.{\land}\@s{7.07} mid \.{\in} Mutex}%
\@x{\@s{12.29} \.{\land}\@s{7.07} pc [ pid ] \.{\in} UnlockIns}%
\@x{\@s{12.29} \.{\land}\@s{7.07} isHead ( pid ,\, waitedQueue [ mid ] )}%
 \@x{\@s{12.29} \.{\land}\@s{7.07} waitedQueue \.{'} \.{=} [ waitedQueue
 {\EXCEPT} {\bang} [ mid ] \.{=} Tail ( waitedQueue [ mid ] ) ]}%
 \@x{\@s{12.29} \.{\land}\@s{7.07} \E\, ins \.{\in} Instr \.{:} pc \.{'} \.{=}
 [ pc {\EXCEPT} {\bang} [ pid ] \.{=} ins ]}%
 \@x{\@s{12.29} \.{\land}\@s{7.07} {\UNCHANGED} {\langle} memory ,\, request
 {\rangle}}%
\@pvspace{8.0pt}%
\@x{}%
\@y{\@s{0}%
 \ensuremath{Mtest \.{=} Test\_any
}}%
\@xx{}%
\@pvspace{8.0pt}%
\@x{ Mtest ( pid ,\, reqs ,\, test\_r ) \.{\defeq}}%
\@x{\@s{8.2} \.{\land}\@s{5.93} pid \.{\in} Proc}%
\@x{\@s{8.2} \.{\land}\@s{5.93} pc [ pid ] \.{\in} MwaitIns}%
\@x{\@s{8.2} \.{\land}\@s{5.93} test\_r \.{\in} Addr}%
\@x{\@s{8.2} \.{\land}\@s{5.93} reqs \.{\subseteq} Addr}%
 \@x{\@s{8.2} \.{\land}\@s{5.93} \.{\lor} \E\, req \.{\in} reqs ,\, r \.{\in}
 request \.{:} r . id \.{=} memory [ pid ] [ req ]}%
\@x{\@s{36.35} \.{\land} isHead ( pid ,\, waitedQueue [ r . mid ] )}%
 \@x{\@s{36.35} \.{\land} request \.{'} \.{=} ( request \.{\,\backslash\,} \{
 r \} )}%
 \@x{\@s{36.35} \.{\land} memory \.{'} \.{=} [ memory {\EXCEPT} {\bang} [ pid
 ] [ test\_r ] \.{=} ValTrue ]}%
 \@x{\@s{25.24} \.{\lor} {\lnot} \E\, req \.{\in} reqs ,\, r \.{\in} reqs
 \.{:} r . id \.{=} memory [ pid ] [ req ]}%
\@x{\@s{36.35} \.{\land} isHead ( pid ,\, waitedQueue [ r . mid ] )}%
 \@x{\@s{36.35} \.{\land} memory \.{'} \.{=} [ memory {\EXCEPT} {\bang} [ pid
 ] [ test\_r ] \.{=} ValFalse ]}%
 \@x{\@s{8.2} \.{\land}\@s{5.93} \E\, ins \.{\in} Instr \.{:} pc \.{'} \.{=} [
 pc {\EXCEPT} {\bang} [ pid ] \.{=} ins ]}%
\@pvspace{16.0pt}%
\@x{}%
\@y{\@s{0}%
 \ensuremath{Mwait \.{=} wait\_any
}}%
\@xx{}%
\@pvspace{8.0pt}%
\@x{ Mwait ( pid ,\, reqs ) \.{\defeq}}%
\@x{\@s{8.2} \.{\land}\@s{8.74} pid \.{\in} Proc}%
\@x{\@s{8.2} \.{\land}\@s{8.74} pc [ pid ] \.{\in} MwaitIns}%
 \@x{\@s{8.2} \.{\land}\@s{8.74} \E\, req \.{\in} reqs ,\, r \.{\in} request
 \.{:} r . id \.{=} memory [ pid ] [ req ]}%
\@x{\@s{35.28} \.{\land} isHead ( pid ,\, waitedQueue [ r . mid ] )}%
 \@x{\@s{35.28} \.{\land} request \.{'} \.{=} ( request \.{\,\backslash\,} \{
 r \} )}%
 \@x{\@s{8.2} \.{\land}\@s{8.74} \E\, ins \.{\in} Instr \.{:} pc \.{'} \.{=} [
 pc {\EXCEPT} {\bang} [ pid ] \.{=} ins ]}%
\@pvspace{16.0pt}%
\@x{ Init\@s{4.12} \.{\defeq}}%
 \@x{\@s{33.24} \.{\land} waitedQueue \.{=} [ i \.{\in} Mutex \.{\mapsto}
 {\langle} {\rangle} ]}%
 \@x{\@s{33.24} \.{\land} pc \.{=} {\CHOOSE} f \.{\in} [ Proc \.{\rightarrow}
 Instr ] \.{:} {\TRUE}}%
 \@x{\@s{33.24} \.{\land} memory\@s{4.1} \.{\in} [ Proc \.{\rightarrow} [
 Mutex \.{\rightarrow} Nat ] ]}%
\@pvspace{16.0pt}%
 \@x{ Next \.{\defeq} \exists\, p \.{\in} Proc ,\, mutex \.{\in} Mutex ,\, req
 \.{\in} Addr ,\, test\_r \.{\in} Addr \.{:}}%
\@x{\@s{48.03} \.{\lor} Ilock ( p ,\, mutex ,\, req )}%
\@x{\@s{48.03} \.{\lor} Unlock ( p ,\, mutex )}%
\@x{\@s{48.03} \.{\lor} Mwait ( p ,\, mutex )}%
\@x{\@s{48.03} \.{\lor} Mtest ( p ,\, mutex ,\, test\_r )}%
\@pvspace{32.0pt}%
 \@x{ Spec\@s{1.46} \.{\defeq} Init \.{\land} {\Box} [ Next ]_{ {\langle} pc
 ,\, waitedQueue ,\, memory {\rangle}}}%
\@pvspace{8.0pt}%
\@x{}\bottombar\@xx{}%
\setboolean{shading}{false}
\begin{lcom}{0}%
...
\begin{cpar}{0}{F}{F}{0}{0}{}%
 ...
 \ensuremath{Jan} 19 16:43:16 \ensuremath{CET} 2018 by diep-chi
\end{cpar}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 \ensuremath{\.{\,\backslash\,}}* Created \ensuremath{Thu} \ensuremath{Dec} 14
 10:59:35 \ensuremath{CET} 2017 by diep-chi
\end{cpar}%
\end{lcom}%
\end{document}
